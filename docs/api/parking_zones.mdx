...
Для зон партнёров рекомендуется использовать:
...
- `location_type` (`string | null`) — тип расположения зоны. Рекомендуемые значения:
  - `street`
  - `open_lot`
  - `underground`
  - `multilevel`
...
> Поля `partner_id`, `created_by_user_id`, `is_active`, `location_type`, `is_accessible`, `confidence_level` являются дополнительными.
...
- `zone_type` (`string`) — фильтр по типу парковки.
