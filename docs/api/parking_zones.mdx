...
- `min_confidence` (`integer`) — фильтр по достоверности оценки занятости.
- `max_pay` (`integer`) — фильтр по максимальной стоимости парковки в час.
- `show_private` (`boolean`) — возвращать ли частные или ограниченно доступные парковки.
- `show_accessible` (`boolean`) — возвращать ли инвалидные парковки.
...
В случае отсутствия определенных данных о парковке (например, инвалидная она или нет), при указании соответствующих фильтров, возвращать ее не следует.
